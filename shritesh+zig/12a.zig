...
const Map = struct {
    ...
    fn countPaths(self: Map) usize {
        ...
        self.visit(&visited, &count, start);

        return count;
    }

    fn visit(self: Map, visited: *Visited, count: *usize, current: usize) void {
        if (current == end) {
            count.* += 1;
            return;
        }
        ...
        while (edges.next()) |e| {
            if (self.isSmall(e) and visited.isSet(e)) continue;
            self.visit(visited, count, e);
        }

        visited.unset(current);
    }
};
